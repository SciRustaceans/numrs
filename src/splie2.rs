...
pub fn splie2(x1a: &[f64], x2a: &[f64], ya: &Array2<f64>) -> Array2<f64> {
...
pub fn splie2_parallel(x1a: &[f64], x2a: &[f64], ya: &Array2<f64>) -> Array2<f64> {
    assert_eq!(x1a.len(), ya.nrows(), "x1a length must match ya rows");
    assert_eq!(x2a.len(), ya.ncols(), "x2a length must match ya cols");
...
fn spline(x: &[f64], y: ArrayView1<f64>, yp1: f64, ypn: f64, mut y2: ArrayViewMut1<f64>) {
    let n = x.len();
...
fn splint(x: &[f64], y: &Array1<f64>, y2: &Array1<f64>, x_val: f64) -> f64 {
...
fn splint_interpolate(x: &[f64], y: &[f64], y2: &[f64], x_val: f64) -> f64 {
    let h = x[1] - x[0];
    let a = (x[1] - x_val) / h;
    let b = (x_val - x[0]) / h;
...
pub fn spline2_interpolate(x1a: &[f64], x2a: &[f64], ya: &Array2<f64>, x1: f64, x2: f64) -> f64 {
...
mod tests {
    ...
    fn test_splie2_constant_function() {
        let x1a = [0.0, 1.0, 2.0];
        let x2a = [0.0, 1.0, 2.0];
        let ya = array![[5.0, 5.0, 5.0], [5.0, 5.0, 5.0], [5.0, 5.0, 5.0]];
    ...
    fn test_spline2_at_grid_points() {
        let x1a = [0.0, 1.0, 2.0];
        let x2a = [0.0, 1.0, 2.0];
        let ya = array![[0.0, 1.0, 4.0], [1.0, 2.0, 5.0], [4.0, 5.0, 8.0]];
