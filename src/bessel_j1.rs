...
impl BesselJ1Cache {
    ...
    /// Get bessj1 value with caching (quantized input)
...
pub fn bessj1_batch_parallel(x_values: &[f64]) -> Vec<f64> {
    x_values.par_iter()
        .map(|&x| bessj1(x))
        .collect()
}
...
pub fn bessj1_range_parallel(start: f64, end: f64, num_points: usize) -> Vec<(f64, f64)> {
...
/// Compute the ratio J₁(x)/J₀(x) efficiently
pub fn bessj1_over_j0(x: f64) -> Option<f64> {
    if x == 0.0 {
        return Some(0.0);
    }
    ...
    let j0 = bessj0(x);
    if j0.abs() < 1e-12 {
        None // Avoid division by zero near zeros of J₀
    } else {
        Some(bessj1(x) / j0)
    }
}

/// Compute the derivative of J₀(x) = -J₁(x)
pub fn bessj0_derivative(x: f64) -> f64 {
    -bessj1(x)
}

/// Compute the derivative of J₁(x) = J₀(x) - J₁(x)/x
pub fn bessj1_derivative(x: f64) -> f64 {
    if x == 0.0 {
        0.5 // Special case: J₁'(0) = 1/2
    } else {
        bessj0(x) - bessj1(x) / x
    }
}
...
mod tests {
    ...
    fn test_bessj1_basic() {
        ...
        assert_relative_eq!(bessj1(5.0), -0.3275791376, epsilon = 1e-6);
    }
    ...
    #[test]
    fn test_bessj1_zeros() {
        // Test approximate locations of first few zeros
        // First zero: ~3.8317
        assert!(bessj1(3.8317).abs() < 1e-4);
        
        // Second zero: ~7.0156
        assert!(bessj1(7.0156).abs() < 1e-4);
        
        // Third zero: ~10.1735
        assert!(bessj1(10.1735).abs() < 1e-4);
    }

    #[test]
    fn test_bessj1_algorithm_boundary() {
        // Test continuity at the algorithm boundary (x = 8.0)
        let left = bessj1(7.999);
        let right = bessj1(8.001);
        assert_relative_eq!(left, right, epsilon = 1e-6);
    }

    #[test]
    fn test_bessj0_basic() {
        // Test J₀ for completeness
    ...
    #[test]
    fn test_bessj1_over_j0() {
        // Test the ratio J₁/J₀
        let ratio = bessj1_over_j0(1.0).unwrap();
        assert_relative_eq!(ratio, 0.4400505857 / 0.7651976866, epsilon = 1e-6);
        
        // Test near zero of J₀
        assert!(bessj1_over_j0(2.4048).is_none()); // First zero of J₀
    }
    ...
    fn test_derivatives() {
        ...
        // Test J₀'(x) = -J₁(x)
        let j0_deriv_approx = (bessj0(x + h) - bessj0(x - h)) / (2.0 * h);
        assert_relative_eq!(bessj0_derivative(x), j0_deriv_approx, epsilon = 1e-6);
        assert_relative_eq!(bessj0_derivative(x), -bessj1(x), epsilon = 1e-10);
        
        // Test J₁'(x) = J₀(x) - J₁(x)/x
        let j1_deriv_approx = (bessj1(x + h) - bessj1(x - h)) / (2.0 * h);
        assert_relative_eq!(bessj1_derivative(x), j1_deriv_approx, epsilon = 1e-6);
    }
    ...
    fn test_parallel_batch() {
        ...
        let results = bessj1_batch_parallel(&x_values);
        
        for (i, &result) in results.iter().enumerate() {
            let expected = bessj1(x_values[i]);
...
mod bench {
    ...
    fn bench_bessj1_small_x(c: &mut Criterion) {
        c.bench_function("bessj1(1.0)", |b| {
            b.iter(|| bessj1(black_box(1.0)))
        });
        
        c.bench_function("bessj1(5.0)", |b| {
            b.iter(|| bessj1(black_box(5.0)))
        });
    }

    fn bench_bessj1_large_x(c: &mut Criterion) {
        c.bench_function("bessj1(10.0)", |b| {
            b.iter(|| bessj1(black_box(10.0)))
        });
        
        c.bench_function("bessj1(100.0)", |b| {
            b.iter(|| bessj1(black_box(100.0)))
        });
    }

    fn bench_bessj0(c: &mut Criterion) {
        c.bench_function("bessj0(2.0)", |b| {
            b.iter(|| bessj0(black_box(2.0)))
        });
    }
    ...
    fn bench_batch_parallel(c: &mut Criterion) {
        ...
        c.bench_function("bessj1_batch_parallel(100)", |b| {
            b.iter(|| bessj1_batch_parallel(black_box(&x_values)))
        });
    }
    ...
    fn bench_cache(c: &mut Criterion) {
        ...
        c.bench_function("bessj1_cached(1.0)", |b| {
    ...
    criterion_group!(
        benches, 
        bench_bessj1_small_x, 
        bench_bessj1_large_x,
        bench_bessj0,
        bench_bessjn,
