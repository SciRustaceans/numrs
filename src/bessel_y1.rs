...
const BESSEL_Y1_BOUNDARY: f64 = 8.0;
...
fn bessy1_small_x(x: f64) -> Result<f64, String> {
    ...
    let j1_part = 0.636619772 * (bessel_j1::bessj1(x) * x.ln() - 1.0 / x);
...
impl BesselY1Cache {
    ...
    /// Get bessy1 value with caching (quantized input)
...
pub fn bessy1_batch_parallel(x_values: &[f64]) -> Vec<Result<f64, String>> {
    x_values.par_iter()
        .map(|&x| bessy1(x))
        .collect()
}
...
pub fn bessy1_range_parallel(start: f64, end: f64, num_points: usize) -> Vec<Result<(f64, f64), String>> {
    if start <= 0.0 {
...
pub fn bessy0(x: f64) -> Result<f64, String> {
    ...
    if x < 8.0 {
        ...
        let rational_part = num / den;
        let j0_part = 0.636619772 * bessj0(x) * x.ln();
...
/// Compute the ratio Y₁(x)/Y₀(x) efficiently
pub fn bessy1_over_y0(x: f64) -> Result<f64, String> {
    if x <= 0.0 {
        return Err("x must be positive".to_string());
    }
    ...
    if y0.abs() < 1e-12 {
        Err("Division by zero near zeros of Y₀".to_string())
    } else {
        Ok(y1 / y0)
    }
}

/// Compute the derivative of Y₀(x) = -Y₁(x)
pub fn bessy0_derivative(x: f64) -> Result<f64, String> {
    bessy1(x).map(|y1| -y1)
}

/// Compute the derivative of Y₁(x) = Y₀(x) - Y₁(x)/x
pub fn bessy1_derivative(x: f64) -> Result<f64, String> {
    if x == 0.0 {
        Err("Derivative undefined at x = 0".to_string())
    } else {
        let y0 = bessy0(x)?;
        let y1 = bessy1(x)?;
        Ok(y0 - y1 / x)
...
mod tests {
    ...
    #[test]
    fn test_bessy1_error_cases() {
        // Test error cases
        assert!(bessy1(0.0).is_err());
        assert!(bessy1(-1.0).is_err());
    }
    ...
    #[test]
    fn test_bessy1_algorithm_boundary() {
        // Test continuity at the algorithm boundary (x = 8.0)
        let left = bessy1(7.999).unwrap();
        let right = bessy1(8.001).unwrap();
        assert_relative_eq!(left, right, epsilon = 1e-6);
    }

    #[test]
    fn test_bessy0_basic() {
        // Test Y₀ for completeness
    ...
    #[test]
    fn test_bessy1_over_y0() {
        // Test the ratio Y₁/Y₀
        let ratio = bessy1_over_y0(2.0).unwrap();
        let y0 = bessy0(2.0).unwrap();
        let y1 = bessy1(2.0).unwrap();
        assert_relative_eq!(ratio, y1 / y0, epsilon = 1e-10);
        
        // Test near zero of Y₀
        assert!(bessy1_over_y0(0.893).is_err()); // Near first zero of Y₀
    }

    #[test]
    fn test_derivatives() {
        // Test derivatives using finite differences
        let x = 2.0;
        let h = 1e-6;
        
        // Test Y₀'(x) = -Y₁(x)
        let y0_deriv_approx = (bessy0(x + h).unwrap() - bessy0(x - h).unwrap()) / (2.0 * h);
        assert_relative_eq!(bessy0_derivative(x).unwrap(), y0_deriv_approx, epsilon = 1e-6);
        assert_relative_eq!(bessy0_derivative(x).unwrap(), -bessy1(x).unwrap(), epsilon = 1e-10);
        
        // Test Y₁'(x) = Y₀(x) - Y₁(x)/x
        let y1_deriv_approx = (bessy1(x + h).unwrap() - bessy1(x - h).unwrap()) / (2.0 * h);
        assert_relative_eq!(bessy1_derivative(x).unwrap(), y1_deriv_approx, epsilon = 1e-6);
    }
    ...
    fn test_parallel_batch() {
        ...
        let results = bessy1_batch_parallel(&x_values);
        
        for (i, result) in results.iter().enumerate() {
            assert!(result.is_ok());
            let expected = bessy1(x_values[i]).unwrap();
    ...
    fn test_cache_functionality() {
        ...
        assert_relative_eq!(result1, bessy1(1.0).unwrap(), epsilon = 1e-10);
    }
...
mod bench {
    ...
    fn bench_bessy1_small_x(c: &mut Criterion) {
        c.bench_function("bessy1(1.0)", |b| {
            b.iter(|| bessy1(black_box(1.0)).unwrap())
        });
        
        c.bench_function("bessy1(5.0)", |b| {
            b.iter(|| bessy1(black_box(5.0)).unwrap())
        });
    }

    fn bench_bessy1_large_x(c: &mut Criterion) {
        c.bench_function("bessy1(10.0)", |b| {
            b.iter(|| bessy1(black_box(10.0)).unwrap())
        });
        
        c.bench_function("bessy1(100.0)", |b| {
            b.iter(|| bessy1(black_box(100.0)).unwrap())
        });
    }

    fn bench_bessy0(c: &mut Criterion) {
        c.bench_function("bessy0(2.0)", |b| {
            b.iter(|| bessy0(black_box(2.0)).unwrap())
        });
    }
    ...
    fn bench_batch_parallel(c: &mut Criterion) {
        ...
        c.bench_function("bessy1_batch_parallel(100)", |b| {
            b.iter(|| bessy1_batch_parallel(black_box(&x_values)))
        });
    }
    ...
    fn bench_cache(c: &mut Criterion) {
        ...
        c.bench_function("bessy1_cached(1.0)", |b| {
    ...
    criterion_group!(
        benches, 
        bench_bessy1_small_x, 
        bench_bessy1_large_x,
        bench_bessy0,
        bench_bessyn,
